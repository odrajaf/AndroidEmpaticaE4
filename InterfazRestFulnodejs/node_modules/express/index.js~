...
var db = mongoskin.db("mongodb://@localhost:27017/empaticaBD", {safe:true});
...
app.get('/api/:coleccion/email', auth, function(req, res, next) { req.collection.findOne({
	"email": req.query.email }, function(e, result){
	if (e) return next(e);
	res.send(result);
	});
 });
...
app.post('/api/:coleccion', auth, function(req, res, next){ 
	req.collection.insert( req.body, function(e, result){
		if (e) return next(e);
			res.send((result != null) ? {msg:'success'} : {msg:'error'});
	}); 
});
